...
Item {
    ...
    // The host shell owns settings, paths and panel lifecycle. Keeping those
    // behind this deliberately small object lets the Slack state/agent layer
    // run unchanged in Noctalia, DMS, end-4 and Caelestia.
    property var platformAdapter: null
    readonly property var cfg: platformAdapter?.settings || ({})
    readonly property var defaults: platformAdapter?.defaults || ({})

    // Host-provided rendering tokens. Views use their shell's native theme;
    // the backend only needs these values to render Slack mrkdwn to rich text.
    property var renderPalette: ({
        link: "#8ab4f8",
        mention: "#8ab4f8",
        mentionSelf: "#c4b5fd",
        code: "#f0abfc",
    ...
    property string fixedFontFamily: "monospace"
    property int renderedEmojiSize: 18
    ...
    readonly property var renderColors: ({
            link: root.renderPalette.link,
            mention: root.renderPalette.mention,
            mentionSelf: root.renderPalette.mentionSelf,
            code: root.renderPalette.code,
            quote: root.renderPalette.quote,
            monoFamily: root.fixedFontFamily,
            emojiSize: root.renderedEmojiSize,
            muted: String(root.renderPalette.muted)
        })
    ...
    function pluginDir() {
        return platformAdapter?.pluginDir || ".";
    }
    ...
    function cacheDir() {
        if (platformAdapter?.cacheDir)
            return platformAdapter.cacheDir;
    ...
    function _parse(text, context) {
        ...
        if (raw === "") {
            ...
            console.error("Slack: " + root.lastError);
            return null;
        }
        try {
            ...
            console.error("Slack: " + root.lastError + " — " + e + " — " + raw.slice(0, 200));
            return null;
        }
    }
    ...
    function switchIdentity(pref) {
        if (!platformAdapter || pref === root.tokenPreference)
            return;
        platformAdapter.setSetting("tokenPreference", pref);
    ...
    function togglePin(id) {
        ...
        if (at >= 0)
            ...
            list.push(id);
        platformAdapter.setSetting("pinned", list);
        poll();
    }
    ...
    function notify(title, body, iconPath) {
        ...
        if (iconPath)
            // Notification daemons prefer the image hint over the app icon and
            // require a real file — hence the mirrored avatars.
